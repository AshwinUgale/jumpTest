defmodule JumpTestWeb.CounterLive.FormComponent do
  use JumpTestWeb, :live_component
  ...
  def update(%{counter: counter} = assigns, socket) do
    {:ok,
     ...
     |> assign(assigns)
     |> assign_new(:form, fn ->
       to_form(Demo.change_counter(counter))
     end)}
  end

  @impl true
  def handle_event("validate", %{"counter" => counter_params}, socket) do
    changeset = Demo.change_counter(socket.assigns.counter, counter_params)
  ...
  defp save_counter(socket, :edit, counter_params) do
    case Demo.update_counter(socket.assigns.counter, counter_params) do
      {:ok, counter} ->
